    ...
    public class Building_TentDoor : Building_Door
    {
        ...
        protected void DrawMovers(Vector3 drawPos, float offsetDist, float altitude, Vector3 drawScaleFactor, Graphic_Shadow shadowGraphic)
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                Graphics.DrawMesh(mesh, Matrix4x4.TRS(vector2, base.Rotation.AsQuat, new Vector3((float)this.def.size.x * drawScaleFactor.x, drawScaleFactor.y, (float)this.def.size.z * drawScaleFactor.z)), this.MoverGraphic[i].MaterialFor(this, this.Position, linkSet), 0);
