...
namespace PortableBlueprint.Tent
{
    public class WorkGiver_RemoveTentRoof : WorkGiver_RemoveRoof
    {
        ...
        {
            return pawn.Map.areaManager.Get<Area_NoTentRoof>()[c] && (c.GetRoof(pawn.Map) == PB_DefOf.PB_TentRoof) && !c.IsForbidden(pawn) && pawn.CanReserve(c, 1, -1, ReservationLayerDefOf.Ceiling, forced);
        }

        public override Job JobOnCell(Pawn pawn, IntVec3 c, bool forced = false)
        {
            return JobMaker.MakeJob(PB_DefOf.PB_RemoveTentRoof, c, c);
        }
