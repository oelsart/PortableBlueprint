using CarryableBlueprint.CB_HarmonyPatch;
using CarryableBlueprint.Tent.TentRoof;
...
namespace CarryableBlueprint.Tent
{
    public class TentPoleComp : ThingComp
    {
        ...
        public override void PostDeSpawn(Map map)
        {
            ...
            {
                IntVec3 intVec = this.parent.Position + GenRadial.RadialPattern[i];
                if (intVec.InBounds(map) && intVec.GetRoof(map) == CB_DefOf.CB_TentRoof && !TentRoofUtility.WithinRangeOfRoofHolder(intVec, map, false, this.parent))
                {
                    RoofDef roof = null;
