...
{
    public override Material MatSingle => MaterialAtlasPool.SubMaterialFromAtlas(((subGraphic as Graphic_Tent)
    ...
    {
        if (subGraphic is not Graphic_Tent)
        {
    ...
    public void Print(SectionLayer layer, Thing thing, float extraRotation, float overrideAltitude, LinkDirections? linkSet = null)
    {
        if (linkSet == null)
        {
            linkSet = GetLinkSet(thing, thing.Position);
        }
        Material mat = MaterialFor(thing, thing.Position, (LinkDirections)linkSet);
        var vector = thing.TrueCenter();
    ...
    {
        return MaterialAtlasPool.SubMaterialFromAtlas((subGraphic as Graphic_Tent).SubGraphicFor(Graphic_Tent.suffixes[0]).MatSingleFor(thing), LinkDirections.None);
    }
    ...
    {
        var linkSet = GetLinkSet(parent, cell);
        return MaterialFor(parent, cell, linkSet);
    }
    ...
    public Material MaterialFor(Thing parent, IntVec3 cell, LinkDirections linkSet)
    {
        ...
        var graphicTent = subGraphic as Graphic_Tent;
        if (num1 == 0 && num2 == 0)
        {
