...
public static class TentRoofUtility
{
    ...
    public static Building GetFirstRoofHolder(IntVec3 c, Map map, bool assumeNonNoRoofCellsAreRoofed = false, Thing ignoreThing = null)
    {
        ...
        map.floodFiller.FloodFill(c, x => (x.GetRoof(map) == PB_DefOf.PB_TentRoof || x == c || (assumeNonNoRoofCellsAreRoofed && !map.areaManager.Get<Area_NoTentRoof>()[x])) && x.InHorDistOf(c, RoofMaxSupportDistance), delegate (IntVec3 x)
        {
            for (int i = 0; i < 5; i++)
            {
    ...
    public static bool ConnectedToRoofHolder(IntVec3 c, Map map, bool assumeRoofAtRoot)
    {
        ...
        map.floodFiller.FloodFill(c, x => (x.GetRoof(map) == PB_DefOf.PB_TentRoof || (x == c & assumeRoofAtRoot)) && !connected, delegate (IntVec3 x)
        {
            for (int i = 0; i < 5; i++)
            {
                IntVec3 c2 = x + GenAdj.CardinalDirectionsAndInside[i];
